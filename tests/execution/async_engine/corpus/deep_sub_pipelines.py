from typing import AsyncIterator, NamedTuple

from synaflow import include, pipeline, step
from tests.common.pipeline_pack import PipelinePack


# Level 3 (Deepest)
class Level3Params(NamedTuple):
    x: int


async def l3_process(x: int) -> int:
    return x * 2


pipe_l3 = pipeline(
    name="Level3",
    params=Level3Params,
    exports="l3_process",
    steps=[step("l3_process", fn=l3_process)],
)


# Level 2 (Middle)
class Level2Params(NamedTuple):
    y: int


async def prep_l3(y: int) -> Level3Params:
    return Level3Params(x=y + 1)


async def l2_process(l3_res: int) -> int:
    return l3_res + 10


pipe_l2 = pipeline(
    name="Level2",
    params=Level2Params,
    exports="l2_process",
    steps=[
        include("l3_res", pipeline=pipe_l3, fn=prep_l3),
        step("l2_process", fn=l2_process),
    ],
)


# Level 1 (Top)
class Level1Params(NamedTuple):
    values: list[int]


async def prep_l2_each(values: list[int]) -> AsyncIterator[Level2Params]:
    for v in values:
        yield Level2Params(y=v)


async def prep_l2_single(values: list[int]) -> Level2Params:
    # Just take the first element for the single sub-pipeline example
    return Level2Params(y=values[0])


async def consolidate(l2_each: list[int], l2_single: int) -> dict:
    return {"each_res": sum(l2_each), "single_res": l2_single}


pipe_l1 = pipeline(
    name="DeepSubPipelines",
    params=Level1Params,
    steps=[
        include("l2_each", pipeline=pipe_l2, fn=prep_l2_each),
        include("l2_single", pipeline=pipe_l2, fn=prep_l2_single),
        step("consolidate", fn=consolidate),
    ],
)

pack = PipelinePack(
    json_dag={
        "name": "DeepSubPipelines",
        "params": {"values": "list[int]"},
        "steps": {
            "l2_each__adapter": {
                "deps": {"values": "list[int]"},
                "output": "Stream[Level2Params]",
                "fn": "prep_l2_each",
                "on_error": "stop",
                "mode": "all",
                "materializer": "memory_materializer",
                "error_materializer": "log_error_materializer",
                "materialized_deps": ["values"],
                "each_mode_deps": [], "max_in_flight": 1,
                "pipeline": "DeepSubPipelines",
                "parent_pipeline": None,
            },
            "l2_each__l3_res__adapter": {
                "deps": {"l2_each__adapter": "Level2Params"},
                "output": "ListType(<class "
                "'tests.execution.async_engine.corpus.deep_sub_pipelines.Level3Params'>)",
                "fn": "prep_l3",
                "on_error": "stop",
                "mode": "each",
                "materializer": "memory_materializer",
                "error_materializer": "log_error_materializer",
                "materialized_deps": ["l2_each__adapter"],
                "each_mode_deps": ["l2_each__adapter"], "max_in_flight": 1,
                "pipeline": "Level2",
                "parent_pipeline": "DeepSubPipelines",
            },
            "l2_each__l3_res": {
                "deps": {"l2_each__l3_res__adapter": "Level3Params"},
                "output": "ListType(<class 'int'>)",
                "fn": "l3_process",
                "on_error": "continue",
                "mode": "each",
                "materializer": "memory_materializer",
                "error_materializer": "log_error_materializer",
                "materialized_deps": ["l2_each__l3_res__adapter"],
                "each_mode_deps": ["l2_each__l3_res__adapter"], "max_in_flight": 1,
                "pipeline": "Level2",
                "parent_pipeline": "DeepSubPipelines",
            },
            "l2_each": {
                "deps": {"l2_each__l3_res": "int"},
                "output": "ListType(<class 'int'>)",
                "fn": "l2_process",
                "on_error": "continue",
                "mode": "each",
                "materializer": "memory_materializer",
                "error_materializer": "log_error_materializer",
                "materialized_deps": [],
                "each_mode_deps": ["l2_each__l3_res"], "max_in_flight": 1,
                "pipeline": "Level2",
                "parent_pipeline": "DeepSubPipelines",
            },
            "l2_single__adapter": {
                "deps": {"values": "list[int]"},
                "output": "Level2Params",
                "fn": "prep_l2_single",
                "on_error": "stop",
                "mode": "all",
                "materializer": "memory_materializer",
                "error_materializer": "log_error_materializer",
                "materialized_deps": ["values"],
                "each_mode_deps": [], "max_in_flight": 1,
                "pipeline": "DeepSubPipelines",
                "parent_pipeline": None,
            },
            "l2_single__l3_res__adapter": {
                "deps": {"l2_single__adapter": "Level2Params"},
                "output": "Level3Params",
                "fn": "prep_l3",
                "on_error": "stop",
                "mode": "all",
                "materializer": "memory_materializer",
                "error_materializer": "log_error_materializer",
                "materialized_deps": ["l2_single__adapter"],
                "each_mode_deps": [], "max_in_flight": 1,
                "pipeline": "Level2",
                "parent_pipeline": "DeepSubPipelines",
            },
            "l2_single__l3_res": {
                "deps": {"l2_single__l3_res__adapter": "Level3Params"},
                "output": "int",
                "fn": "l3_process",
                "on_error": "continue",
                "mode": "all",
                "materializer": "memory_materializer",
                "error_materializer": "log_error_materializer",
                "materialized_deps": ["l2_single__l3_res__adapter"],
                "each_mode_deps": [], "max_in_flight": 1,
                "pipeline": "Level2",
                "parent_pipeline": "DeepSubPipelines",
            },
            "l2_single": {
                "deps": {"l2_single__l3_res": "int"},
                "output": "int",
                "fn": "l2_process",
                "on_error": "continue",
                "mode": "all",
                "materializer": "memory_materializer",
                "error_materializer": "log_error_materializer",
                "materialized_deps": [],
                "each_mode_deps": [], "max_in_flight": 1,
                "pipeline": "Level2",
                "parent_pipeline": "DeepSubPipelines",
            },
            "consolidate": {
                "deps": {"l2_each": "list[int]", "l2_single": "int"},
                "output": "dict",
                "fn": "consolidate",
                "on_error": "continue",
                "mode": "all",
                "materializer": "memory_materializer",
                "error_materializer": "log_error_materializer",
                "materialized_deps": ["l2_each"],
                "each_mode_deps": [], "max_in_flight": 1,
                "pipeline": "DeepSubPipelines",
                "parent_pipeline": None,
            },
        },
        "error_materializer": "log_error_materializer",
    },
    pipeline=pipe_l1,
    input_params=Level1Params(values=[10, 20]),
    step_results={
        "l2_each__adapter": [Level2Params(y=10), Level2Params(y=20)],
        "l2_single__adapter": Level2Params(y=10),
        "l2_each__l3_res__adapter": [Level3Params(x=11), Level3Params(x=21)],
        "l2_single__l3_res__adapter": Level3Params(x=11),
        "l2_each__l3_res": [22, 42],
        "l2_single__l3_res": 22,
        "l2_each": [32, 52],
        "l2_single": 32,
        "consolidate": {"each_res": 84, "single_res": 32},
    },
)
