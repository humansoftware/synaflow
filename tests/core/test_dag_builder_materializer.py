...
def test_given_step_level_materializer_when_dag_built_then_step_materializer_wins():
    ...
    p = build_minimal_dag(
        producer_fn=gen, consumer_fn=consumer, producer_materializer=my_mat_wrapped
    )
    assert build_dag(p).steps["producer"].materializer is my_mat_wrapped
...
def test_given_pipeline_level_factory_when_dag_built_then_factory_stored():
    ...
    p = build_minimal_dag(
        producer_fn=gen, consumer_fn=consumer, pipeline_materializer=my_factory
    )
    assert build_dag(p).steps["producer"].materializer is list
...
def test_given_scalar_producer_when_dag_built_then_materializer_is_default_factory():
    ...
    assert getattr(build_dag(p).steps["producer"].materializer, "__name__", "") in (
        "_identity",
        "list",
        "<lambda>",
        "",
...
def test_given_builtin_concrete_materializer_when_signature_not_inspectable_then_dag_builds():
    ...
    assert build_dag(p) is not None
    assert build_dag(p).steps["producer"].materializer is int
