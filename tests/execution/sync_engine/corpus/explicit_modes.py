from collections.abc import Generator
from typing import NamedTuple

from synaflow import StepMode, pipeline, step
from tests.common.pipeline_pack import PipelinePack


class ExplicitModesParams(NamedTuple):
    items: list[int] = [1, 2, 3]


def emit(items: list[int]) -> Generator[int, None, None]:
    yield from items


def double(emit: int) -> int:
    return emit * 2


def summarize(double: list[int]) -> int:
    return sum(double)


explicit_modes_pipeline = pipeline(
    name="explicit_modes",
    params=ExplicitModesParams,
    steps=[
        step("emit", fn=emit),
        step("double", fn=double, mode=StepMode.EACH),
        step("summarize", fn=summarize, mode=StepMode.ALL),
    ],
)

pack = PipelinePack(
    json_dag={
        "name": "explicit_modes",
        "params": {"items": "list[int]"},
        "steps": {
            "emit": {
                "deps": {"items": "list[int]"},
                "output": "Stream[int, None, None]",
                "fn": "emit",
                "on_error": "continue",
                "mode": "all",
                "materializer": "memory_materializer",
                "error_materializer": "log_error_materializer",
                "materialized_deps": ["items"],
                "each_mode_deps": [], "max_in_flight": 1,
                "pipeline": "explicit_modes",
                "parent_pipeline": None,
            },
            "double": {
                "deps": {"emit": "int"},
                "output": "ListType(<class 'int'>)",
                "fn": "double",
                "on_error": "continue",
                "mode": "each",
                "materializer": "memory_materializer",
                "error_materializer": "log_error_materializer",
                "materialized_deps": [],
                "each_mode_deps": ["emit"], "max_in_flight": 1,
                "pipeline": "explicit_modes",
                "parent_pipeline": None,
            },
            "summarize": {
                "deps": {"double": "list[int]"},
                "output": "int",
                "fn": "summarize",
                "on_error": "continue",
                "mode": "all",
                "materializer": "memory_materializer",
                "error_materializer": "log_error_materializer",
                "materialized_deps": ["double"],
                "each_mode_deps": [], "max_in_flight": 1,
                "pipeline": "explicit_modes",
                "parent_pipeline": None,
            },
        },
        "error_materializer": "log_error_materializer",
    },
    pipeline=explicit_modes_pipeline,
    input_params=ExplicitModesParams(items=[1, 2, 3]),
    step_results={
        "emit": [1, 2, 3],
        "double": [2, 4, 6],
        "summarize": 12,
    },
    expected_execution_levels=[["emit"], ["double"], ["summarize"]],
)
