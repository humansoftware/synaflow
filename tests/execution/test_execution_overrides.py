...
def test_given_materializer_override_when_sync_run_then_override_is_used(run_pipeline):
    ...
    p = pipeline(
        name="sync_override",
        params=Params,
        steps=[step("items", fn=gen), step("consume", fn=consume)],
    )
...
async def test_given_materializer_override_when_async_run_then_override_is_used():
    ...
    await async_run(build_dag(p), Params(), overrides=overrides)
    assert captured == [(0, 1, 2)]
...
def test_given_unknown_materializer_override_key_when_assigned_then_raises():
    ...
    p = pipeline(
        name="invalid_override_key", params=Params, steps=[step("emit", fn=emit)]
    )
    overrides = ExecutionOverrides.empty(p)
...
def test_given_invalid_observer_override_value_when_assigned_then_raises():
    ...
    with pytest.raises(
        TypeError, match="must contain only callables or Observer registrations"
    ):
...
def test_given_scope_key_when_materializer_overridden_in_sub_pipeline_then_override_is_used(
    ...
    sub = pipeline(
        name="sub",
        params=SubParams,
        steps=[step("prepare", fn=prepare), step("finish", fn=finish)],
        exports="finish",
    )
    ...
    p = pipeline(
        ...
        params=Params,
        steps=[include("incl", pipeline=sub, fn=adapt), step("consume", fn=consume)],
    )
    overrides = ExecutionOverrides.empty(p)
