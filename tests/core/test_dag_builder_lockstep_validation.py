...
def test_original_case_4_only_s2_materializes_throws_design_time_error():
    ...
    def s3(s2: list[int]) -> Iterator[int]:
        yield len(s2)  # Yields to become a stream again
    ...
    with pytest.raises(ValueError, match="Asymmetric lockstep materialization"):
        pipeline(
            ...
            steps=[
                ...
                step("s2b", s2b, mode=StepMode.EACH),
                step("s3", s3, mode=StepMode.ALL),  # Takes list, returns iterator
                step("s3b", s3b, mode=StepMode.EACH),
                step("s4", s4, mode=StepMode.EACH),
            ],
        )
...
def test_original_case_5_s2_and_s3b_materialize_validates_successfully():
    ...
    my_pipe = pipeline(
        ...
        steps=[
            ...
            step(
                "s4", s4, mode=StepMode.EACH
            ),  # Takes one item from s3 stream, and the scalar list from s3b
        ],
    )
    assert my_pipe.dag is not None
    ...
    with pytest.raises(ValueError, match="Asymmetric lockstep materialization"):
        pipeline(
            ...
            steps=[
                ...
                step(
                    "s3c4", s3c4, mode=StepMode.EACH
                ),  # Consumes stream from s3c2, and scalar list from s3c1. Returns stream implicitly via EACH mapping
                step("s4", s4, mode=StepMode.EACH),
            ],
        )
